            ...
            {recentBills.length === 0 ? (
              ...
              <div className="space-y-2">
                {recentBills.map((bill: any) => (
                  <div key={bill.id || bill._id} className="flex items-center justify-between p-3 rounded-xl bg-muted/30 hover:bg-muted/50 transition-colors">
                    <div>
                      <p className="font-medium text-sm">{bill.billNumber}</p>
                      <p className="text-xs text-muted-foreground">{bill.createdAt ? format(new Date(bill.createdAt), "dd MMM, hh:mm a") : ""}</p>
                    </div>
